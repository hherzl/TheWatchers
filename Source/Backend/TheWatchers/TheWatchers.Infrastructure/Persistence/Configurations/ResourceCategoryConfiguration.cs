...
internal class ResourceCategoryConfiguration : EntityConfiguration<ResourceCategory>
{
    public override void Configure(EntityTypeBuilder<ResourceCategory> builder)
    {
        ...
        builder.Property(p => p.WatcherId).HasColumnType("smallint");

        // Add configuration for uniques

        builder.HasIndex(p => p.Name).IsUnique().HasDatabaseName("UQ_dbo_ResourceCategory_Name");

        // Add configuration for foreign keys

        builder
            .HasOne(p => p.Watcher)
            .WithMany(b => b.ResourceCategories)
